...
pressctl is an automated WordPress hosting infrastructure using Ansible. It consists of:
...
**Note**: Users typically interact via the CLI (`press` command), but you can run Ansible directly for testing or advanced usage.
...
```bash
cd ansible
ansible-playbook website.yml -i "SERVER_IP," -u pressctl \
...
cd ansible
# Add domain
ansible-playbook playbooks/domain_management.yml -i "IP," -u pressctl \
...
```bash
cd ansible
ansible-playbook playbooks/delete_site.yml -i "IP," -u pressctl \
...
- **bootstrap**: Creates `pressctl` system user, installs base packages, certbot, fail2ban, redis
- **database**: Installs MariaDB, creates `pressctlbot` admin user
...
**Module Path:** `github.com/pressctl/cli`
...
- All state stored in `~/.pressctl/wpsh.yaml`
...
| ------------------------ | ------------------------------------------------------- |
| `pressctl_ssh_key`           | SSH public key for wpsh user (file path or key content) |
