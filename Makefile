...
# Build CLI
build:
	@echo "Building pressctl CLI (v$(VERSION))..."
	@cd cli && make build
	@echo "✓ Build complete: cli/press"

# Install CLI to /usr/local/bin (requires sudo)
install: build
	@echo "Installing pressctl CLI to /usr/local/bin..."
	@cd cli && make install
	@echo ""
	@echo "✓ Installation complete!"
	@echo ""
	@echo "Next step: Run 'press init' to set up your environment"

# Install CLI to ~/bin (no sudo required)
install-user: build
	@echo "Installing pressctl CLI to ~/bin..."
...
test-ansible:
	...
	@cd ansible && ansible-playbook --syntax-check playbooks/php_version_add.yml
...
help:
	...
	@echo ""
	@echo "Quick start:"
	@echo "  make build && ./cli/press init"
