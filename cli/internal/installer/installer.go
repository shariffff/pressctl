...
func GetWordmonDir() string {
	...
	return filepath.Join(home, pressctlDir)
}

// GetAnsibleDir returns the path to ~/.pressctl/ansible/
func GetAnsibleDir() string {
	return filepath.Join(GetWordmonDir(), ansibleDir)
}

// IsInitialized checks if ~/.pressctl/ansible/ exists and has content
...
func DetectAnsibleSource() (string, error) {
	...
	systemPath := "/usr/local/share/pressctl/ansible"
...
// Initialize sets up the ~/.pressctl directory and copies ansible files
func Initialize() error {
	pressctlPath := GetWordmonDir()
	ansiblePath := GetAnsibleDir()

	// Create ~/.pressctl/ directory
	if err := os.MkdirAll(pressctlPath, 0755); err != nil {
		return fmt.Errorf("failed to create %s: %w", pressctlPath, err)
	}
...
// Checks in order: ~/.pressctl/ansible/, /usr/local/share/pressctl/ansible/, relative path
