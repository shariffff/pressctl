...
func PromptInitSetup() (*InitInput, error) {
	...
	fmt.Println()
	fmt.Println("Let's configure some one-time settings for your pressctl installation.")
	fmt.Println()
	...
	if err != nil || len(sshPubKeys) == 0 {
		...
		keyPrompt := &survey.Input{
			Message: "SSH public key file (for pressctl user):",
			Default: defaultKeyPath,
			Help:    "Path to the SSH public key that will be authorized for the pressctl user on servers",
		}
		...
		keyPrompt := &survey.Select{
			Message: "SSH public key (for pressctl user):",
			Options: options,
			Help:    "Select the SSH public key to authorize for the pressctl user on servers",
		}
