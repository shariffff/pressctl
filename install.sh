...
# pressctl Installer
# Usage: curl -fsSL https://raw.githubusercontent.com/shariffff/pressctl/main/install.sh | bash

REPO="shariffff/pressctl"
BINARY_NAME="press"

# Install directory (like Bun's ~/.bun)
install_dir="${PRESSCTL_INSTALL:-$HOME/.pressctl}"
...
setup_shell() {
    ...
    case $shell_name in
        fish)
            ...
            shell_export="set --export PRESSCTL_INSTALL \"$install_dir\"\nset --export PATH \$PRESSCTL_INSTALL/bin \$PATH"
            ;;
        zsh)
            config_file="$HOME/.zshrc"
            shell_export="export PRESSCTL_INSTALL=\"$install_dir\"\nexport PATH=\"\$PRESSCTL_INSTALL/bin:\$PATH\""
            ;;
        bash)
        ...
        *)
            ...
            echo ""
            echo "  export PRESSCTL_INSTALL=\"$install_dir\""
            echo "  export PATH=\"\$PRESSCTL_INSTALL/bin:\$PATH\""
            echo ""
            return
            ;;
    esac
    ...
    if [[ -f "$config_file" ]] && grep -q "PRESSCTL_INSTALL" "$config_file" 2>/dev/null; then
        # Already configured, just export for current session
        export PRESSCTL_INSTALL="$install_dir"
...
install_pressctl() {
    ...
    local version="${PRESSCTL_VERSION:-$(get_latest_version)}"

    if [[ -z "$version" ]]; then
        error "Could not determine latest version. Set PRESSCTL_VERSION manually."
    fi

    # Remove 'v' prefix if present for filename
    local version_num="${version#v}"

    echo -e "${DIM}Installing pressctl ${version} (${os}/${arch})${NC}"

    # Construct download URL
    local archive_name="pressctl_${version_num}_${os}_${arch}"
...
main() {
    echo ""
    echo -e "${BOLD}pressctl${NC} Installer"
    echo ""
